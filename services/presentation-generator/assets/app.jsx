...
const hasValue = (v) => {
  if (v == null || v === "" || v === "0" || v === 0 || v === false || v === "Нет") return false;
  if (Array.isArray(v)) return v.length > 0;
  return true;
};
const hasNumber = (v) => hasValue(v) && !Number.isNaN(Number(v));
      ...
      <div className="hero-info">
        ...
        <div className="hero-price-block">
          ...
          <div className="hero-price-meta">
            {hasNumber(p.area_m2) && hasNumber(p.price) && p.property_type === "rent" && (
              <span>≈ <b>{fmtMoney(Math.round(p.price * 12 / p.area_m2), p.currency)}</b>/{p.area_unit}/год</span>
            )}
            {hasNumber(p.area_m2) && hasNumber(p.price) && p.property_type === "sale" && (
              <span>≈ <b>{fmtMoney(Math.round(p.price / p.area_m2), p.currency)}</b>/{p.area_unit}</span>
            )}
            {hasValue(p.commission) && <span>Комиссия: <b>{p.commission}</b></span>}
          </div>
        </div>
        ...
        <div className="hero-stats">
          ...
          {hasNumber(p.floor) && (
            <div className="hero-stat">
              ...
              <div className="val">{p.floor}{hasNumber(p.floors_total) && <small>/ {p.floors_total}</small>}</div>
            </div>
          )}
          {hasNumber(p.rooms) && (
            <div className="hero-stat">
          ...
          {hasNumber(p.ceiling_height) && !hasNumber(p.rooms) && (
            <div className="hero-stat">
              <div className="lbl">Потолки</div>
              <div className="val">{p.ceiling_height}<small>м</small></div>
            </div>
          )}
          {hasValue(p.office_class) && !hasNumber(p.floor) && (
            <div className="hero-stat">
              <div className="lbl">Класс</div>
              <div className="val">{p.office_class}</div>
            </div>
          )}
          {!hasNumber(p.rooms) && !hasNumber(p.ceiling_height) && !hasValue(p.office_class) && hasNumber(p.built_year) && (
            <div className="hero-stat">
        ...
        <div className="hero-tags">
          {hasValue(p.office_class) && <span className="tag accent">Класс {p.office_class}</span>}
          {hasValue(p.quality) && <span className="tag">{p.quality}</span>}
          {hasValue(p.renovation) && <span className="tag">{p.renovation}</span>}
          {p.twenty_four_seven && <span className="tag">24/7</span>}
          {hasValue(p.parking) && <span className="tag">Парковка</span>}
        </div>
      </div>
...
function Finance({ p }) {
  const cells = [];
  if (hasNumber(p.price)) {
    cells.push({ lbl: "Цена", val: fmtMoney(p.price, p.currency), sub: p.property_type === "rent" ? "в месяц" : "к оплате" });
  }
  if (hasNumber(p.area_m2) && hasNumber(p.price) && p.property_type === "rent") {
    cells.push({ lbl: "Ставка", val: fmtMoney(Math.round(p.price * 12 / p.area_m2), p.currency), sub: `за ${p.area_unit}/год` });
  }
  if (hasNumber(p.area_m2) && hasNumber(p.price) && p.property_type === "sale") {
    cells.push({ lbl: "Цена за метр", val: fmtMoney(Math.round(p.price / p.area_m2), p.currency), sub: `за ${p.area_unit}` });
  }
  if (hasValue(p.commission)) cells.push({ lbl: "Комиссия", val: p.commission, sub: "" });
  if (hasValue(p.prepayment)) cells.push({ lbl: "Предоплата", val: p.prepayment, sub: "" });
  if (hasValue(p.security_payment)) cells.push({ lbl: "Обеспечительный платёж", val: p.security_payment, sub: "" });
  if (hasNumber(p.deposit_months)) cells.push({ lbl: "Депозит", val: `${p.deposit_months} мес`, sub: "" });
  if (hasValue(p.sale_type)) cells.push({ lbl: "Тип сделки", val: p.sale_type, sub: "" });
  if (hasNumber(p.estimated_payback_years)) cells.push({ lbl: "Окупаемость", val: `${p.estimated_payback_years} лет`, sub: "" });
  if (hasNumber(p.profitability)) cells.push({ lbl: "Доходность", val: `${p.profitability}%`, sub: "годовых" });
...
function KeyParams({ p }) {
  const specs = [];
  const push = (k, v) => hasValue(v) && specs.push([k, v]);

  push("Площадь", hasNumber(p.area_m2) && `${fmtNum(p.area_m2)} ${p.area_unit}`);
  push("Жилая площадь", hasNumber(p.living_space) && `${p.living_space} м²`);
  push("Кухня", hasNumber(p.kitchen_space) && `${p.kitchen_space} м²`);
  push("Комнат", p.rooms);
  push("Этаж", hasNumber(p.floor) && (hasNumber(p.floors_total) ? `${p.floor} из ${p.floors_total}` : p.floor));
  push("Высота потолков", hasNumber(p.ceiling_height) && `${p.ceiling_height} м`);
  push("Планировка", p.layout);
  push("Открытая площадка", hasNumber(p.open_area) && `${p.open_area} м²`);
...
function Specs({ p }) {
  ...
  const filtered = groups
    .map(g => ({ ...g, items: g.items.filter(([_, v]) => hasValue(v)) }))
  ...
  return (
    <section className="block" data-screen-label="Specs">
      ...
      <div className="spec-groups" style={{ "--spec-cols": Math.min(filtered.length, 3) }}>
        {filtered.map((g, gi) => (
...
function Calculator({ p }) {
  ...
  if (!hasNumber(p.price)) return null;
...
function AllData({ p }) {
  ...
  const items = Object.entries(p).filter(([k, v]) => !SHOWN.has(k) && hasValue(v) && typeof v !== "object");
  if (items.length === 0) {
    ...
    return (
      <section className="block" data-screen-label="Meta">
        ...
        <dl className="meta-list">
          {hasValue(p.internal_id) && <div className="meta-item"><dt>Внутренний ID</dt><dd>{p.internal_id}</dd></div>}
          {hasValue(p.quickdeal_id) && <div className="meta-item"><dt>QuickDeal ID</dt><dd>{p.quickdeal_id}</dd></div>}
          {hasValue(p.source_url) && <div className="meta-item"><dt>Источник</dt><dd><a href={p.source_url} style={{ color: "var(--accent)" }}>Ссылка</a></dd></div>}
          {hasValue(p.published_at) && <div className="meta-item"><dt>Опубликовано</dt><dd>{p.published_at}</dd></div>}
          {hasValue(p.updated_at) && <div className="meta-item"><dt>Обновлено</dt><dd>{p.updated_at}</dd></div>}
          {hasValue(p.cadastral_number) && <div className="meta-item"><dt>Кадастр</dt><dd>{p.cadastral_number}</dd></div>}
        </dl>
      </section>
    );
  }
  ...
  return (
    <aside className="sticky-panel">
      <div className="price-card">
        ...
        <div className="price">{hasNumber(p.price) ? fmtMoney(p.price, p.currency) : "по запросу"}</div>
        <div className="period">{periodLabel || (hasNumber(p.area_m2) && hasNumber(p.price) ? `${fmtMoney(Math.round(p.price / p.area_m2), p.currency)} / ${p.area_unit}` : "")}</div>
      </div>
...
const ACCENT_PALETTE = {
  ...
  "#b88750": { deep: "#6f8f82" },  // residential bronze + sage
...
function App() {
  ...
  const p = PROPERTIES[t.property] || PROPERTIES.generated || Object.values(PROPERTIES)[0];

  // apply theme + accent + density + fonts to document
  useEffect(() => {
    const root = document.documentElement;
    const isResidential = p.category === "residential";
    const accent = isResidential && t.accent === TWEAK_DEFAULTS.accent ? "#b88750" : t.accent;
    root.dataset.theme = isResidential ? "light" : (t.dark ? "dark" : "light");
    root.dataset.propertyCategory = isResidential ? "residential" : "commercial";
    root.dataset.density = t.density;
    const accentDeep = ACCENT_PALETTE[accent]?.deep || "#8a6f3e";
  ...
  }, [p.category, t.dark, t.accent, t.density, t.fontPair]);
